...
```{r}
d_votes = c(62, 55, 44, 47, 46)
meanmedianscore <- mean(d_votes) - median(d_votes)
meanmedianscore
```
...
eg <- function(d_votes, r_votes) {
    total <- d_votes + r_votes
    wasted_d_votes <- d_votes - (total %/% 2)
    wasted_r_votes <- r_votes - (total %/% 2)

    totalwasted <- sum(wasted_d_votes) + sum(wasted_r_votes)
    eg <- totalwasted / sum(total)

    return(eg)
}

egvalue <- eg(d_votes, r_votes)
egvalue
```